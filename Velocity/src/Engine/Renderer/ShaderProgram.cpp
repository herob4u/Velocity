...
std::string ShaderProgram::getShaderFile(const char* file)
{
...
void ShaderProgram::checkCompilationStatus (
    ...
    if (compileSuccess == GL_FALSE) {
        ...
        GLchar* errorMessage = (GLchar*)malloc(sizeof(GLchar) * (errorMessageLength + 1)); // Add 1 for null terminator
...
void ShaderProgram::checkLinkStatus() {
    ...
    if (linkSuccess == GL_FALSE) {
        ...
        GLchar* errorMessage = (GLchar*)malloc(sizeof(GLchar) * errorMessageLength);
 ...
 */
int ShaderProgram::getAttribLocation (
